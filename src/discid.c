...
int crip_fill_discid(cyanrip_ctx *ctx)
{
    ...
    snprintf(temp, sizeof(temp), "%02X", ctx->tracks[ctx->last_audio_track_idx].number);
    av_sha_update(sha_ctx, temp, strlen(temp));

    lsn_t last = ctx->tracks[ctx->last_audio_track_idx].end_lsn + 151;
    ...
    for (int i = 0; i < 99; i++) {
        ...
        if (!ctx->tracks[i].track_is_data)
            offset = ctx->tracks[i].start_lsn + (i <= ctx->last_audio_track_idx)*150;
    ...
    /* FreeDB */
    uint32_t cddb = 0;
    for (int i = 0; i <= ctx->last_audio_track_idx; i++) {
        if (ctx->tracks[i].track_is_data)
            continue;
    ...
    cddb |= ctx->tracks[ctx->last_audio_track_idx].number;
    ...
    av_bprintf(&buf, "%i%s%i%s%u", ctx->tracks[0].number, "+",
               ctx->tracks[ctx->last_audio_track_idx].number, "+", last);

    for (int i = 0; i <= ctx->last_audio_track_idx; i++) {
        if (ctx->tracks[i].track_is_data)
            continue;

        uint32_t offset = ctx->tracks[i].start_lsn + (i <= ctx->last_audio_track_idx)*150;
        av_bprintf(&buf, "%s%u", "+", offset);
    }

    av_bprintf(&buf, "&tracks=%i", ctx->last_audio_track_idx + 1);
