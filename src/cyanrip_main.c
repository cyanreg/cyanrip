...
repeat_ripping:
    ...
    if (ctx->settings.ripping_retries) {
        ...
        if (total_repeats >= ctx->settings.ripping_retries) {
            cyanrip_log(ctx, 0, "\nDone; (no matches found, but repeat limit of %i hit)\n",
                        ctx->settings.frame_max_retries);
            goto finalize_ripping;
        }
        ...
        if ((matches + 1) >= ctx->settings.ripping_retries ||
            (total_repeats + 1) >= ctx->settings.frame_max_retries)
            repeat_encode = 1;
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "hNAUfHIVQEWOl:a:t:b:c:r:d:o:s:S:D:p:C:R:P:F:L:T:M:Z:")) != -1) {
        ...
        case 'h':
            ...
            cyanrip_log(ctx, 0, "    -r <int>              Maximum number of retries (default: 25)\n");
